...
import org.eclipse.cdt.dsf.gdb.service.IGDBGrouping.IGroupRenameEvent;
...
/**
 * This VMNode represents a group.
 */
public class GroupVMNode extends AbstractContainerVMNode implements IElementLabelProvider, IElementMementoProvider {
	...
	protected IElementLabelProvider createLabelProvider() {
		...
		provider.setColumnInfo(PropertiesBasedLabelProvider.ID_COLUMN_NO_COLUMNS,
				new LabelColumnInfo(new LabelAttribute[] {
						...
						/* ACTIVE GROUP LABEL */
						new GdbExecutionContextLabelText(MessagesForGdbLaunchVM.GroupVMNode_No_columns__text_format,
	...
	protected void updateElementsInSessionThread(final IChildrenUpdate update) {
		...
		IContainerDMContext container = findDmcInPath(update.getViewerInput(), update.getElementPath(),
				...
				new ViewerDataRequestMonitor<IExecutionDMContext[]>(getExecutor(), update) {
					...
					public void handleCompleted() {
						...
						if (execDmcs != null) {
							...
							fillUpdateWithVMCs(update, groupDmcs.toArray(new IGroupDMContext[groupDmcs.size()]));
						}
						update.done();
					}
				});
	...
	protected void updatePropertiesInSessionThread(IPropertiesUpdate[] updates) {
		...
		for (int i = 0; i < updates.length; i++) {
			...
			final ViewerCountingRequestMonitor countingRm = new ViewerCountingRequestMonitor(
					ImmediateExecutor.getInstance(), update) {
			...
			if (update.getProperties().contains(PROP_NAME)) {
				...
				if (execTranslator == null) {
					...
					execTranslator.getExecutionData(groupDmc,
	...
	protected void fillThreadSummary(final IPropertiesUpdate update, final RequestMonitor rm) {
		...
		final IGroupDMContext groupDmc = findDmcInPath(update.getViewerInput(), update.getElementPath(),
				IGroupDMContext.class);
		...
		runControl.getExecutionContexts(groupDmc,
	...
	@Override
	public void getContextsForEvent(final VMDelta parentDelta, final Object event,
									...
									new DataRequestMonitor<IGroupDMContext[]>(getExecutor(), rm) {
										...
										public void handleCompleted() {
											if (isSuccess()) {
												...
												List<IVMContext> contextsForEvent = new ArrayList<>();
												...
												rm.setData(contextsForEvent
														.toArray(new IVMContext[contextsForEvent.size()]));
											} else {
												rm.setData(new IVMContext[0]);
	...
	public int getDeltaFlags(Object e) {
		if (e instanceof IGroupCreatedEvent || e instanceof IGroupDeletedEvent || e instanceof IGroupChangedEvent) {
			return IModelDelta.CONTENT;
		}
		//<CUSTOMISATION - ASHLING> Handle Group Rename Event
		if (e instanceof IGroupRenameEvent) {
	...
	public void buildDelta(Object e, final VMDelta parentDelta, final int nodeOffset,
		...
		if (e instanceof IGroupChangedEvent) {
		...
		if (e instanceof IGroupCreatedEvent || e instanceof IGroupDeletedEvent) {
			parentDelta.setFlags(parentDelta.getFlags() | IModelDelta.CONTENT);
			requestMonitor.done();
			return;
		}
					...
					if (dmc instanceof IGroupDMContext) {
						try {
							getSession().getExecutor().execute(new DsfRunnable() {
								@Override
								public void run() {
									final IGDBGrouping execTranslator = getServicesTracker()
											.getService(IGDBGrouping.class);
									if (execTranslator != null) {
										execTranslator.getExecutionData((IGroupDMContext) dmc,
												new ViewerDataRequestMonitor<IGroupDMData>(execTranslator.getExecutor(),
														request) {
													@Override
													protected void handleCompleted() {
														if (isSuccess()) {
															memento.putString(MEMENTO_NAME,
																	"Group." + getData().getName() + getData().getId()); //$NON-NLS-1$
														}
														request.done();
													}
												});
